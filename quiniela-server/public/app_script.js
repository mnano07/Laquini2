...
const UI={tab:'apuestas',authMode:'login',openMatch:null,slip:{picks:[],stake:10},casino:{game:null},
  rl:{stake:5,picks:[],spinning:false,history:[],angle:0},bj:null,fanDate:null,fan:null,expandedTx:{}};
...
function showGroupSelect(){
  ...
  injectFloaters(gv);
  gv.querySelectorAll('[data-group]').forEach(b=>b.onclick=()=>pickGroup(b.dataset.group));
}
async function pickGroup(g){
  try{ await api('POST','/api/group',{group:g}); confettiBurst(); await refresh(); setTimeout(showApp,650); }
...
function viewMatchMenu(){
  let html=`<h2 class="section-title">Partidos del Mundial</h2><p class="section-sub">Toca un partido para ver todos los mercados. La lista y los resultados se actualizan solos. Apuestas: abren el día antes y cierran al empezar.</p>`;
  const groups={};S.matches.forEach(m=>{const dk=dayKey(m.kickoff);(groups[dk]=groups[dk]||[]).push(m);});
  for(const dk of Object.keys(groups)){
    html+=`<div class="day-head">${esc(dk)}</div><div class="card" style="margin-bottom:6px">`;
    html+=groups[dk].map(m=>{const sc=m.finished?`<span class="menu-score">${m.result.hg} – ${m.result.ag}</span>`:'';const t=new Date(m.kickoff).toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'});
      return `<div class="menu-match" data-openmatch="${m.id}"><div class="menu-teams"><div class="tt"><span class="flag">${flag(m.home)}</span>${esc(m.home)} <span style="color:var(--muted)">vs</span> <span class="flag">${flag(m.away)}</span>${esc(m.away)}</div><div class="mt">${t} · ${esc(m.group||'')} ${m.venue?'· '+esc(m.venue):''}</div></div>${sc}${statusPill(m.status)}<span class="chev">›</span></div>`;}).join('');
    html+=`</div>`;
  }
  return html;
}
...
/* ===== MIS APUESTAS ===== */
function viewMisApuestas(){const mine=S.myBets;let head=`<h2 class="section-title">Mis apuestas</h2><p class="section-sub">Las combinadas solo pagan si aciertas todas. Se liquidan solas con los resultados.</p>`;if(!mine.length)return head+`<div class="card empty">Aún no has apostado.</div>`;const ico={won:'✅',lost:'❌',pending:'⏳'};
  const rows=mine.map(b=>{const legs=b.selections.map((s,i)=>`<div class="leg-line"><span class="ic">${ico[b.legStates[i]]}</span><div class="lt"><b>${esc(s.selLabel)}</b><div class="mm">${esc(s.marketLabel||MK_LABEL[s.market])} · ${esc(s.match)}</div></div><span class="lo">@ ${s.odd.toFixed(2)}</span></div>`).join('');const stTxt=b.status==='pending'?'Pendiente':b.status==='won'?'Ganada +'+money(b.stake*b.totalOdd):'Perdida −'+money(b.stake);
...
/* ===== REGISTRO / HISTORIAL ===== */
function txIcon(t){return ({bet_win:'⚽',bet_lose:'⚽',roulette:'🎡',blackjack:'🃏',bonus:'🎁',admin_adjust:'🛠️'}[t])||'💶';}
...
/* ===== ADMIN ===== */
function viewAdmin(){let head=`<h2 class="section-title">Administración</h2><p class="section-sub">Estás en modo administrador. Telegram ${S.meta.telegram?'✅':'⚪'} · API fútbol ${S.meta.provider?'✅':'⚪'}. <button class="adm-lock" id="admLock">Salir del modo admin</button></p>`;
  ...
  const rows=S.matches.map(m=>{const r=m.result||{};return `<div class="admin-match" data-amid="${m.id}"><div class="nm">${flag(m.home)} ${esc(m.home)} vs ${esc(m.away)} ${flag(m.away)}<div class="section-sub" style="margin:0">${fmtDate(m.kickoff)} ${m.finished?'· ✅':''}</div></div><div class="admin-grid"><label>Goles</label><input class="am-hg" type="number" min="0" value="${r.hg??''}" placeholder="L"><input class="am-ag" type="number" min="0" value="${r.ag??''}" placeholder="V"><label>Tarj.</label><input class="am-hc" type="number" min="0" value="${r.hcards??''}" placeholder="L"><input class="am-ac" type="number" min="0" value="${r.acards??''}" placeholder="V"><label>Córn.</label><input class="am-co" type="number" min="0" value="${r.corners??''}"><label>Parad.</label><input class="am-sv" type="number" min="0" value="${r.saves??''}"><button class="btn btn-gold am-save">${m.finished?'Re-guardar':'Guardar'}</button></div></div>`;}).join('');
  return head+manage+toggle+`<h4 style="font-family:'Bebas Neue';font-size:20px;letter-spacing:.04em;margin:8px 4px">Resultados</h4><div class="card">${rows}</div><p class="section-sub" style="margin-top:10px">Si conectas la API de fútbol, los resultados entran solos.</p>`;}
...
function wireView(){
  ...
  document.querySelectorAll('.admin-match').forEach(row=>{const btn=row.querySelector('.am-save');if(!btn)return;btn.onclick=async()=>{const g=s=>parseInt(row.querySelector(s).value);const body={mid:row.dataset.amid,hg:g('.am-hg'),ag:g('.am-ag'),hcards:g('.am-hc'),acards:g('.am-ac'),corners:g('.am-co'),saves:g('.am-sv')};if(Object.values(body).slice(1).some(x=>isNaN(x)||x<0)){toast('Rellena todos los campos ≥ 0');return;}try{const o=await api('POST','/api/admin/result',body);await refresh();toast(`Guardado · ${o.settled} liquidada(s)`,'win');render();}catch(e){toast(e.message,'lose');}};});
}
