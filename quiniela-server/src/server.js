...
app.get('/api/state', auth, (req,res)=>{
  ...
  const myBets=state.bets.filter(b=>b.user===meName).sort((a,b)=>b.placedAt-a.placedAt)
    .map(b=>({...b, legStates:b.selections.map(s=>{const m=state.matches.find(x=>x.id===s.mid);return (!m||!m.finished)?'pending':(resultFor(m.result,s.market)===s.sel?'won':'lost');})}));
  ...
  res.json({
    ...
    meta:{ demoOpen:state.meta.demoOpen, provider:providerConfigured(), telegram:tgConfigured(), startBalance:START_BALANCE }
  });
});
...
app.post('/api/admin/result', auth, adminOnly, (req,res)=>{
  ...
  const vals=[hg,ag,hcards,acards,corners,saves].map(Number);
  if(vals.some(v=>isNaN(v)||v<0)) return res.status(400).json({error:'Valores no válidos'});
  m.finished=true; m.result={hg:vals[0],ag:vals[1],hcards:vals[2],acards:vals[3],corners:vals[4],saves:vals[5]};
