...
export function resultFor(r, market){
  ...
  if(market==='bothCards')return(r.hcards>=1&&r.acards>=1)?'Sí':'No';
  if(market==='cards'){const c=r.hcards+r.acards;return c<=2?'≤ 2':c<=4?'3 - 4':'5+';}
  if(market==='corners'){const c=r.corners;return c<=8?'≤ 8':c<=11?'9 - 11':'12+';}
  if(market==='saves'){const s=r.saves;return s<=5?'≤ 5':s<=9?'6 - 9':'10+';}
  return null;
}
